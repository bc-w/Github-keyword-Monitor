...
class github_serch:
    ...
    def login_github(self,username="bc-w",password=""):#登陆Github
        ...
        try:
            ...
            for page in range(1,3): #检索1到2页匹配关键词的结果
                search_code = "https://github.com/search?p="+str(page)+"&q=&type=Code"
                ...
                for i in range(len(Urls)):
                    n = n + 1
                    writer.writerow([str1+str(Urls[i]),datetime[i],users[i]])
    ...
    def send_mail(self): #发送邮件
        ...
        for ff in f2: #检索上传时间
            i2 = float(ff.strip("\n"))
            timeStamp2020 = float(1590388819.0)
        ...
        if n1 == 0: #判断上传数量
            try:
                ...
                msg.attach(
                    MIMEText("Dear all: \r\n\r\n"+str(date)+" 检测关键字,Github检测到上传代码"+str(n)+"个,未发现新上传代码，详见附件\r\n\r\n"))
                msg.attach(att1)
                smtp = smtplib.SMTP()
